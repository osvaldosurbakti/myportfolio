...
export default function SkillsCarousel() {
  ...
  const filteredSkills = useMemo(() => {
    ...
    setTimeout(() => setIsLoading(false), 300); // Simulasi loading delay
    return filtered;
  ...
  return (
    <div className="py-16 px-6 bg-gradient-to-b from-blue-50 to-white dark:from-gray-800 dark:to-gray-900">
      <h2 className="text-4xl font-bold text-center text-gray-900 dark:text-white">Skills</h2>
        ...
        {categories.map((cat) => (
          <button
            ...
            className={`px-4 py-2 rounded-full font-semibold transition-all ${
              selectedCategory === cat ? "bg-blue-600 text-white" : "bg-gray-200 dark:bg-gray-700 text-gray-900 dark:text-white"
            }`}
          >
            {cat}
          </button>
              ...
              {filteredSkills.map((skill) => (
                <SwiperSlide key={skill.name}>
                  <Tilt glareEnable={true} glareMaxOpacity={0.5} scale={1.05}>
                    <motion.div
                      className="p-6 bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 relative group text-center"
                    >
                      {/* Lazy Loading untuk Ikon */}
                      ...
                      <span className={`px-3 py-1 text-sm rounded-full text-white ${skillLevels[skill.level as keyof typeof skillLevels]} absolute top-3 right-3`}>
  {skill.level}
</span>                    </motion.div>
                  </Tilt>
                </SwiperSlide>
